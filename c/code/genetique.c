...
void crossover_pmx(int *parent1, int *parent2, int *enfant1, int *enfant2, int size)
{
    ...
    // Remplir le reste en évitant les doublons
    for (int i = 0; i < size; i++)
    {
...
int *genetique(Matrix *distance, List place) // A chaque fois on mets place.size-1 car on enleve la base de la liste principal
{                                            // creation de l'element
    ...
    for (int i = 0; i < POP_INIT; i++)
    {
        fitness[i] = eval_fitness(population[i], distance, place.size - 1);
