                      ...
                      double *min_lat, double *max_lat)
{
    ...
    *max_lat = -90.0;
    ...
    {
        if (places.lst[i].coord.longitude < *min_lon)
            *min_lon = places.lst[i].coord.longitude;
        if (places.lst[i].coord.longitude > *max_lon)
            *max_lon = places.lst[i].coord.longitude;
        if (places.lst[i].coord.latitude < *min_lat)
            *min_lat = places.lst[i].coord.latitude;
        if (places.lst[i].coord.latitude > *max_lat)
            *max_lat = places.lst[i].coord.latitude;
    }
